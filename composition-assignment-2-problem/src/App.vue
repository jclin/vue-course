<template>
  ...
  <section>
    <div>Available Funds: {{ expenseData.availableFunds }}</div>
    <div>Total Expenses: {{ expenseData.currentExpenses }}</div>
    <hr />
...
export default {
  setup() {
    ...
    function addExpense() {
      console.log(enteredExpense.value);
      expenseData.currentExpenses += enteredExpense.value;
    }

    const remainingFunds = computed(function () {
      return expenseData.availableFunds - expenseData.currentExpenses;
    });
